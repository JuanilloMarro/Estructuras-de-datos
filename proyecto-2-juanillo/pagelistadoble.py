...
from structures import lista_doble

lista_doble = lista_doble.DoubleLinkedList()


class PageListaDoble():
	def __init__(self):
		self.top = Toplevel()

		self.top.title('Lista Doblemente Enlazada')
		...
		self.frame_all_lista_doble = Frame(self.top)
		self.frame_all_lista_doble.config(width= 650, height=600, bg='white')
		self.frame_all_lista_doble.pack(side='left', fill="both", expand=True)

		# Frame del título y su respectivo label
		self.frame_titulo_lista_doble = Frame(self.frame_all_lista_doble)
		self.frame_titulo_lista_doble.config(bg='white', width= 650, height=150)
		self.frame_titulo_lista_doble.pack(fill="both", expand=True, padx=100, pady=50)
		self.label_titulo_lista_doble = Label(self.frame_titulo_lista_doble)
		self.label_titulo_lista_doble.config(font=self.fuente, text='Visualización de la Lista Doble', bg='white')
		self.label_titulo_lista_doble.pack()

		# Frame de la construcción de la Lista Simple
		self.frame_lista_doble = Frame(self.frame_all_lista_doble)
		self.frame_lista_doble.config(bg='white', width= 650, height=450)
		self.frame_lista_doble.pack(fill="both", expand=True)

		# Frames que contienen los números que existen dentro de la Lista Simple y su respectivo label
		self.frame_content = Frame(self.frame_lista_doble)
		...
		# --- Información de la Lista Doble ---

		# Frame principal donde se encuentra toda la información de la Lista Doble
		self.frame_info_lista_doble = Frame(self.top)
		self.frame_info_lista_doble.config(bg='#C8EDC8', width=350, height=750)
		self.frame_info_lista_doble.pack(side='right', fill='both')

		# Frame que contiene el título 'Información de la Lista Simple' y todos los frames correspondientes a él
		self.info_especifica_lista_doble = Frame(self.frame_info_lista_doble)
		self.info_especifica_lista_doble.config(bg='#C8EDC8')
		self.info_especifica_lista_doble.pack(pady=15, padx=5)
		self.label_title_info_lista_doble = Label(self.info_especifica_lista_doble,
												  text='Información de la Lista Doble', bg='#C8EDC8',
												  font=self.fuente)
		self.label_title_info_lista_doble.pack()

		# Frame que contiene dos labels: el tamaño y el otro label donde se asigna el tamaño
		self.frame_tamano = Frame(self.info_especifica_lista_doble)
		...
		self.label_tamanoo = Label(self.frame_tamano, text=lista_doble.get_size(), bg='#C8EDC8',
								   font=self.fuente)
		self.label_tamanoo.pack(side='right')

		# Frame que contiene dos labels: el tope y el otro label donde se asigna el tope
		self.frame_tope = Frame(self.info_especifica_lista_doble)
		...
		self.label_topee = Label(self.frame_tope, text=str(lista_doble.get_head()), bg='#C8EDC8',
								 font=self.fuente)
		self.label_topee.pack(side='left')

		# Frame que contiene dos labels: el fondo y el otro label donde se asigna el fondo
		self.frame_fondo = Frame(self.info_especifica_lista_doble)
		...
		self.label_fondoo = Label(self.frame_fondo, text=str(lista_doble.get_tail()), bg='#C8EDC8',
								  font=self.fuente)
		...
		self.opciones_valor = Frame(self.frame_info_lista_doble)
		...
		self.label_titulo_valor = Label(self.frame_valor, text='Ingrese la posición: ', bg='#C8EDC8', font=self.fuente)
		self.label_titulo_valor.pack(side='left')
		self.label_position = Entry(self.frame_valor, font=self.fuente, width=7)
		...
		# Frame que contienen los 3 botones que modifican la visualización de la Lista Doble
		...
		self.button_add_start = Button(self.frame_boton_opcion, text='Add Start', bg='#C8EDC8', font=self.fuente,
									   command=self.add)
		self.button_add_start.pack(side='left', padx=5)
		self.button_add_end = Button(self.frame_boton_opcion, text='Add End', bg='#C8EDC8', font=self.fuente,
									 command=self.add_last)
		self.button_add_end.pack(side='left', padx=5)
		self.button_add_position = Button(self.frame_boton_opcion, text='Add by position', bg='#C8EDC8',
										  font=self.fuente, command=self.add_position)
		self.button_add_position.pack(side='left', padx=5)

		self.frame_boton_opciones = Frame(self.opciones_valor)
		self.frame_boton_opciones.config(bg='#C8EDC8')
		self.frame_boton_opciones.pack(pady=2.5, padx=2.5)
		self.button_delete_start = Button(self.frame_boton_opciones, text='Delete Start', bg='#C8EDC8',
										  font=self.fuente, command=self.delete)
		self.button_delete_start.pack(side='left', padx=5)
		self.button_delete_end = Button(self.frame_boton_opciones, text='Delete End', bg='#C8EDC8', font=self.fuente,
										command=self.delete_last)
		self.button_delete_end.pack(side='left', padx=5)
		self.button_delete_position = Button(self.frame_boton_opciones, text='Delete by position', bg='#C8EDC8',
											 font=self.fuente, command=self.delete_position)
		self.button_delete_position.pack(side='left', padx=5)

		self.frame_boton_opcioness = Frame(self.opciones_valor)
		self.frame_boton_opcioness.config(bg='#C8EDC8')
		self.frame_boton_opcioness.pack(pady=2.5, padx=2.5)
		self.button_search = Button(self.frame_boton_opcioness, text='Search', bg='#C8EDC8', font=self.fuente,
									command=self.search).pack(side='left', padx=5)

		# Cargar archivos

		self.frame_archivos = Frame(self.frame_info_lista_doble)
	...
	def guardar_en_json(self):
		...
		datos = lista_doble.to_list()

		self.file_data[nombre_archivo] = datos

		with open("json/lista_doble.json", "w") as f:
	...
	def actualizar_lista(self):
		...
		with open("json/lista_doble.json", "r") as f:
	...
	def render(self, datos):
		...
		self.frame_lista_doble.destroy()

		self.frame_lista_doble = Frame(self.frame_all_lista_doble)
		self.frame_lista_doble.config(bg='white')
		self.frame_lista_doble.pack(side='left', pady=200)
	...
	def actualizar_info(self):
		self.label_tamanoo.config(text=lista_doble.get_size())
		self.label_topee.config(text=str(lista_doble.get_head()))
		self.label_fondoo.config(text=str(lista_doble.get_tail()))

	def actualizar_lista_doble(self):
		...
		for item in lista_doble.to_list():

			if item == lista_doble.to_list()[0]:
				self.frame_content = Frame(self.frame_lista_doble)
				self.frame_content.config(bg='#C8EDC8', width=7, height=3, bd=2, relief='solid')
				self.frame_content.pack(side='left')
	...
	def add(self):
		...
		self.actualizar_lista_doble()

	def add_last(self):

		self.text = self.label_valor.get()

		lista_doble.append(self.text)
	...
	def add_position(self):
		self.text = self.label_valor.get()
		self.position = self.label_position.get()

		lista_doble.append_in_position(self.text, int(self.position))
	...
	def delete(self):
		...
		lista_doble.remove_head()

		if lista_doble.get_size() == 0:
			self.label_tamanoo.config(text=lista_doble.get_size())
	...
	def delete_position(self):
		...
		lista_doble.remove_in_position(int(self.position))
	...
	def search(self):
		...
		elemento = lista_doble.search(self.text)
